...
<div class="form-card">
    ...
    <form action="{{ route('receitas.update', $receita) }}" method="POST">
        ...
        <input type="text" id="nome" name="nome" value="{{ old('nome', $receita->nome) }}" required>

        <label for="descricao">Descrição</label>
        <textarea id="descricao" name="descricao">{{ old('descricao', $receita->descricao) }}</textarea>

        <label for="data_registro">Data de Registro</label>
        <input type="date" id="data_registro" name="data_registro" value="{{ old('data_registro', $receita->data_registro->format('Y-m-d')) }}" required>

        <label for="custo">Custo (R$)</label>
        <input type="number" step="0.01" min="0" id="custo" name="custo" value="{{ old('custo', $receita->custo) }}" required>
